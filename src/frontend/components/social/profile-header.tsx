...
export function ProfileStatsHeader({
  profile, postCount, projectCount, repoCount, isFollowing, isPending, isSelf, signedIn = true,
  onFollow, onUnfollow, onCancelRequest, onSignInRequired, onCopyId, copied, action,
}: ProfileStatsHeaderProps) {
  const initials = profile.name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2)
  const showFollow = !isSelf && !profile.uid.startsWith("seed-")
